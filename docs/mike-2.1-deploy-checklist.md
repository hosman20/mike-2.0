...
7. **Live AI Gateway smoke not run**: Streaming, tool calls, and cost attribution have not been manually verified end-to-end against a real `AI_GATEWAY_API_KEY`. Run a chat + a tabular generation against staging before flipping production traffic.
8. **Gateway model-slug naming**: `backend/src/lib/llm/gateway.ts` maps to slugs like `anthropic/claude-sonnet-4.6`, `google/gemini-3-flash-preview`, `openai/gpt-5.5`. Verify against `curl -s https://ai-gateway.vercel.sh/v1/models | jq '.data[].id'` before deploy — Vercel may rename preview-suffixed slugs. The skill prerequisite explicitly warns: **always fetch current model IDs**.
